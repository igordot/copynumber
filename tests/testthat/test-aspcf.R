test_that("aspcf()'s documented example produces stable output", {
  ...
  data(logR)
  data(BAF)
  winsLogR <- winsorize(logR, verbose = FALSE)

  aspcfSegments <- aspcf(winsLogR, BAF, verbose = FALSE)

  expect_equal(nrow(aspcfSegments), 218)
  expect_equal(sum(aspcfSegments$n.probes), 20000)
  expect_equal(aspcfSegments$logR.mean[1], -0.2323, tolerance = 0.01)
  expect_equal(aspcfSegments$BAF.mean[1], 0.6916, tolerance = 0.01)
})
...
test_that("aspcf() reproduces the vignette's SNP-array workflow", {
  ...
  for (id in unique(alleleSeg$sampleID)) {
    n <- sum(alleleSeg$n.probes[alleleSeg$sampleID == id])
    expect_equal(n, nrow(logRWins))
  }
})
