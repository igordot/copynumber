...
% Please edit documentation in R/plotAllele.r
\name{plotAllele}
\alias{plotAllele}
...
\arguments{
\item{logR}{a data frame with numeric or character chromosome numbers in the
...
\item{BAF}{a data frame on the same format and size as \code{logR}, with
...
\item{segments}{a data frame or a list of data frames containing the
...
\item{sample}{a numeric vector indicating which sample(s) is (are) to be
...
\item{chrom}{a numeric or character vector with chromosome number(s) to
...
\item{baf.thres}{a numeric vector of length 2 giving thresholds below/above
which BAF-values will not be plotted (use this to remove germline homozygous
BAF probes from the plot).}

\item{winsoutliers}{an optional data frame of the same size as \code{logR}
...
\item{xaxis}{either "pos" or "index". The former implies that the xaxis will
represent the genomic positions, whereas the latter implies that the xaxis
will represent the probe indices. Default is "pos".}

\item{layout}{an integer vector of length two giving the number of rows and
columns in the plot. Default is \code{c(1,1)}.}

\item{plot.ideo}{a logical value indicating whether the chromosome ideogram
should be plotted. Only applicable when \code{xaxis="pos"}.}

\item{\dots}{other graphical parameters. These include the common plot
...
\note{
This function applies \code{par(fig)}, and is therefore not compatible
