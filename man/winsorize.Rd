...
\arguments{
\item{data}{either a data frame or the name of a tab-separated file from
...
\item{pos.unit}{the unit used to represent the probe positions. Allowed
options are "mbp" (mega base pairs), "kbp" (kilo base pairs) or "bp" (base
pairs). By default assumed to be "bp".}

\item{arms}{optional character vector containing chromosome arms (denoted
...
\item{method}{the Winsorization method to be applied, must be one of "mad"
(default) or "pcf".}

\item{tau}{Winsorization threshold, default is 2.5.}

\item{k}{the half window size to be applied in median filtering, default is
25.}

\item{gamma}{penalty for each discontinuity in the pcf curve, default is 40.
Only applicable when \code{method="pcf"}.}

\item{iter}{number of iterations in PCF Winsorization, default is 1.}

\item{assembly}{a string specifying which genome assembly version should be
...
\item{digits}{the number of decimals to be applied when reporting results.
Default is 4.}

\item{return.outliers}{logical value indicating whether a data frame
identifying outliers should be returned, default is FALSE.}

\item{save.res}{logical value indicating whether results should be saved in
text files, default is FALSE.}

\item{file.names}{optional character vector of length two giving the name of
the files where the Winsorized data and outlier statuses, respectively,
should be saved if \code{save.res=TRUE}.}

\item{verbose}{logical value indicating whether or not to print a progress
message each time Winsorization is finished for a new chromosome arm.}
}
\value{
If \code{return.outliers = TRUE} a list with the following
...
\note{
Any missing values in \code{data} imply that the Winsorized value and
...
\references{
Nilsen and Liestoel et al., "Copynumber: Efficient algorithms
