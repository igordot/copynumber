...
#' below (if `large=FALSE`) the threshold is returned. If `thres` is
#' not given by the user, a given number of segments determined by the input in
#' `nseg` is selected; if `large=TRUE` this will be the `nseg`
#' segments with the highest variance, whereas if `large=FALSE` the subset
#' will consist of the `nseg` segments with the lowest variance.
#'
...
#' or below (if `large=FALSE`) this threshold is returned. If `thres`
#' is left unspecified, a given number of segments determined by the input in
#' `nseg` is selected; if `large=TRUE` this will be the `nseg`
#' longest segments, whereas if `large=FALSE` it will be the `nseg`
#' shortest segments.
#'
...
#' the parameter `p` is returned. If `thres` is not specified, the
#' `nseg` segments with the highest (1-p)-quantile (if `large=TRUE`)
#' or the lowest p-quantile (if `large=FALSE`) is returned.
#'
...
#' `what` is "variance", "length" or "aberration", respectively.
...
#' segments.} In addition, depending on the value of `what`:
#' \item{seg.var}{a vector giving the variance for each segment. Only returned
#' if `what = "variance"`.} \item{seg.length}{a vector giving the length
#' of each segment. Only returned if `what = "length"`.}
#' \item{seg.ab.prop}{a vector giving the aberration proportion for each
#' segment. Only returned if `what = "aberration"` and `thres` is
#' specified.} \item{seg.quantile}{a vector giving the (1-p)- or p-quantile for
#' each segment. Only returned if `what = "aberration"` and
#' `thres=NULL`.}
#' @author Gro Nilsen
#' @seealso [multipcf()]
#' @examples
