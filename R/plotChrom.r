...
#' @note This function applies \code{par(fig)}, and is therefore not
#' compatible with other setups for arranging multiple plots in one device
#' such as \code{par(mfrow,mfcol)}.
#' @author Gro Nilsen
  ...
  for (c in 1:nChrom) {
    ...
    if (!arg$onefile || c == 1) {
      ...
      if (!is.null(arg$dir.print)) {
        pdf(
          file = paste(arg$dir.print, "/", file.name[c], ".pdf", sep = ""),
    ...
    for (i in 1:nSample) {
      ...
      if (!is.null(segments)) {
        ...
        for (s in 1:nSeg) {
          ...
          plotSegments(
            use.segments[use.segments[, 1] == id, , drop = FALSE],
            type = "chromosome",
            k = k,
            sampleID = id,
            xaxis = xaxis,
    ...
    if (!is.null(arg$dir.print)) {
      if (!arg$onefile) {
        cat(
          "Plot was saved in ",
          paste(arg$dir.print, "/", file.name[c], ".pdf", sep = ""),
          "\n"
        )
        graphics.off()
      } else {
