...
#' @param BAF a data frame on the same format and size as `logR`, with
...
#' segmentation results found by [aspcf()].
...
#' appearance) in `logR`, or in `segments` if `logR` is
#' unspecified.
...
#' @param winsoutliers an optional data frame of the same size as `logR`
...
plotAllele <- function(
  ...
  arg1 <- modifyList(
    arg,
    list(xlab = "", ylab = arg$ylab[1], h = arg$h[1], mar = mar1)
  )
  arg2 <- modifyList(
    arg,
    list(
  ...
  for (i in 1:nSample) {
    ...
    for (c in 1:nChrom) {
      ...
      logR.frame <- frame.c
      logR.frame$bot <- frame.c$bot +
        (frame.c$top - frame.c$bot) * (baf.frac + arg$ideo.frac)
      baf.frame <- frame.c
      ...
      if (!is.null(segments)) {
        ...
        seg.lim1 <- c(
          min(seg.lim1[1, ], na.rm = TRUE),
          max(seg.lim1[2, ], na.rm = TRUE)
        ...
        seg.lim2 <- c(
          (1 - max(seg.lim2[2, ], na.rm = TRUE)),
          max(seg.lim2[2, ], na.rm = TRUE)
        ) # Get overall min and max over all segments; subtract 0.5 in minimum because we mirror around 0.5 later in plotting
      } else {
        seg.lim1 <- NULL
        seg.lim2 <- NULL
      }
      ...
      if (!is.null(logR)) {
        ...
        # plot logR
        plotObs(
          y = logR[ind.chrom, ind.sample + 2],
          pos = logR[ind.chrom, 2],
          unit = pos.unit,
