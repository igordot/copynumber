...
#' before running `pcf`. Default is TRUE.
...
#' @param \dots other optional parameters to be passed to `pcf`.
#' @return If `cv = TRUE` a list containing: \item{gamma}{the gamma values
  ...
  if (cv) {
    ...
    for (k in 1:K) {
      ...
      for (g in 1:nGamma) {
        ...
        sse[g, k] <- sum(
          (use.data[test.set, 3] - yhat[test.set, 3])^2,
          na.rm = TRUE
        )
      }
